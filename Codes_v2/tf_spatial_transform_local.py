...
def transformer(U, im_one, depth, theta, name='SpatialTransformer', **kwargs):
    ...
    def get_Hs(theta, patch = 512.): 
        with tf.compat.v1.variable_scope('get_Hs'):
            num_batch = tf.shape(input=theta)[0]
            h = patch / grid_h
            w = patch / grid_w
            Hs = []
    ...
    def _meshgrid2(height, width, sh, eh, sw, ew):
        ...
        x_t = tf.matmul(tf.ones(shape=tf.stack([hn, 1])),
                        tf.transpose(a=tf.expand_dims(tf.slice(tf.linspace(-1.0, 1.0, width), [sw], [wn]), 1), perm=[1, 0]))
        ...
        with tf.compat.v1.variable_scope('_transform'):
            ...
            for i in range(grid_h):
                ...
                for j in range(grid_w):
                    ...
                    t_1 = tf.ones(shape = tf.shape(input=z_s_flat))
                    t_0 = tf.zeros(shape = tf.shape(input=z_s_flat))      

                    sign_z_flat = tf.compat.v1.where(z_s_flat >= 0, t_1, t_0) * 2 - 1
