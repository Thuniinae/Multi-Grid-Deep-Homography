...
def depth_consistency_loss3(warp2_depth, mesh):
    ...
    depth_patches = tf.image.extract_patches(warp2_depth, [1,shape[1]/grid_h,shape[2]/grid_w,1], [1,shape[1]/grid_h,shape[2]/grid_w,1], [1,1,1,1], padding='VALID')
    depth_map = tf.reduce_mean(input_tensor=depth_patches, axis=3)
    ...
    cos_w = tf.reduce_sum(input_tensor=w_edges[:,:,0:grid_w-1,:] * w_edges[:,:,1:grid_w,:],axis=3) / (tf.sqrt(tf.reduce_sum(input_tensor=w_edges[:,:,0:grid_w-1,:]*w_edges[:,:,0:grid_w-1,:],axis=3))*tf.sqrt(tf.reduce_sum(input_tensor=w_edges[:,:,1:grid_w,:]*w_edges[:,:,1:grid_w,:],axis=3)))
    ...
    cos_h = tf.reduce_sum(input_tensor=h_edges[:,0:grid_h-1,:,:] * h_edges[:,1:grid_h,:,:],axis=3) / (tf.sqrt(tf.reduce_sum(input_tensor=h_edges[:,0:grid_h-1,:,:]*h_edges[:,0:grid_h-1,:,:],axis=3))*tf.sqrt(tf.reduce_sum(input_tensor=h_edges[:,1:grid_h,:,:]*h_edges[:,1:grid_h,:,:],axis=3)))
    ...
    for i in range(depth_num):
        ...
        depth_mask = tf.compat.v1.where(tf.logical_and((depth_map>start_depth) , (depth_map<=end_depth)), ones, zeros)
        ...
        error.append(tf.reduce_mean(input_tensor=error_w) + tf.reduce_mean(input_tensor=error_h))
