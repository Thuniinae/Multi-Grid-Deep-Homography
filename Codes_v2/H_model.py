...
def H2Mesh(H2, patch_size):
    batch_size = tf.shape(input=H2)[0]
    h = patch_size/grid_h
    w = patch_size/grid_w
    ori_pt = []
    ...
    H2_local = tf.transpose(a=H2_local, perm=[0, 2, 1])
...
def H_model(train_inputs_aug, train_inputs, train_depth, patch_size=512.):
    ...
    batch_size = tf.shape(input=train_inputs)[0]
    ...
    warp2_depth = tf.expand_dims(tf.reduce_mean(input_tensor=warp2_depth, axis=3),3)
...
def CCL(c1, warp):
    ...
    patches = tf.image.extract_patches(warp, [1,kernel,kernel,1], [1,stride,stride,1], [1,rate,rate,1], padding='SAME')
    patches = tf.reshape(patches, [shape[0], -1, kernel, kernel, shape[3]])
    matching_filters = tf.transpose(a=patches, perm=[0, 2, 3, 4, 1])
    ...
    w_one = tf.matmul(tf.ones(shape=tf.stack([shape[1], 1])), tf.transpose(a=tf.expand_dims(w_one, 1), perm=[1, 0]))
    ...
    flow_w = tf.expand_dims(tf.reduce_sum(input_tensor=flow_w,axis=3),3)
    flow_h = tf.expand_dims(tf.reduce_sum(input_tensor=flow_h,axis=3),3)
